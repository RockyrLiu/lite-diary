...
class _ChatPageState extends ConsumerState<ChatPage> {
  ...
  Future<LlmService?> _buildService() =>
      LlmConfigService.buildConfiguredService();
  ...
  Future<String> _buildEntriesContext() async {
    ...
    if (dateStart != null) {
      entries = entries
          .where(
            (e) => e.date.isAfter(dateStart.subtract(const Duration(days: 1))),
          )
          .toList();
    }
    if (dateEnd != null) {
      entries = entries
          .where((e) => e.date.isBefore(dateEnd.add(const Duration(days: 1))))
          .toList();
    }
  ...
  Future<void> _generateTitle(int convId) async {
    ...
    try {
      final prompt =
          '请为以下对话生成一个简短的标题（不超过15个字），只返回标题不要加其他内容：\n${history.first.content}';
      final title = await service.sendMessage(
        messages: [
  ...
  Future<void> _sendMessage({String? overrideContent}) async {
    ...
    if (convId == null) {
      convId = await db.createConversation(
        ConversationsCompanion(
          ...
          groupIds: Value(
            ref.read(conversationGroupIdsProvider).isNotEmpty
    ...
    final portraitText = await PortraitService(db).buildInjectionText();

    final messages = <Map<String, String>>[];
    if (portraitText != null && portraitText.isNotEmpty) {
      messages.add({
        'role': 'system',
        'content': '以下是你对用户的了解（画像仅供参考）：\n\n$portraitText',
      });
    }
    ...
    final assistantMsgId = await db.createMessage(
      MessagesCompanion(
        conversationId: Value(convId),
    ...
    try {
      ...
      _streamSub = stream.listen(
        (chunk) {
          ...
          db.updateMessage(
            assistantMsgId,
            MessagesCompanion(content: Value(buffer.toString())),
          );
          ref.invalidate(messagesByConversationProvider(convId!));
          _scrollToBottom();
        },
        ...
        onError: (e) async {
          ...
          await db.updateMessage(
            assistantMsgId,
            MessagesCompanion(content: Value('[错误] ${e.toString()}')),
          );
          ref.invalidate(messagesByConversationProvider(convId!));
          setState(() {});
        },
  ...
  void _selectConversation(int id) {
    ...
    ref.read(databaseProvider).getConversationById(id).then((conv) {
      if (conv != null && mounted) {
        ...
        if (conv.dateStart != null) {
          ref.read(conversationDateStartProvider.notifier).state =
              conv.dateStart;
        } else {
          ref.read(conversationDateStartProvider.notifier).state = null;
        }
        if (conv.dateEnd != null) {
          ref.read(conversationDateEndProvider.notifier).state = conv.dateEnd;
        } else {
          ref.read(conversationDateEndProvider.notifier).state = null;
        }
        if (conv.groupIds != null && conv.groupIds!.isNotEmpty) {
          ref.read(conversationGroupIdsProvider.notifier).state = conv.groupIds!
              .split(',')
              .map(int.parse)
              .toList();
        } else {
      ...
      body: Column(
        children: [
          Container(
            ...
            color: Theme.of(
              context,
            ).colorScheme.surfaceContainerHighest.withValues(alpha: 0.39),
            child: Row(
              children: [
                Expanded(
                  child: Text(
                ...
                IconButton(
                  ...
                  onPressed: () => showDialog(
                    context: context,
                    builder: (_) => const DataScopeSelector(),
                  ),
                ),
                IconButton(
          ...
          Expanded(
            child: convId == null
                ...
                : ref
                      .watch(messagesByConversationProvider(convId))
                      .when(
                        data: (messages) {

...
| 分组管理 | 默认"日记/随笔/诗词"三分组 + 自定义 CRUD |
| 标签系统 | 多对多关键词标签，彩色底色 Chip，仅编辑模式下操作 |
| 那年今日 | 往年同日日记随机回顾，翻页浏览 |
| 即时搜索 | 实时筛选，标题 + 全文匹配 |
| 图片插入 | 从相册选图插入 Markdown，拷贝到应用私有目录 |
| 农历日历 | lunar package，干支纪年 + 农历月日 (1900-2100) |
| 诗稿导出 | 诗词分组一键导出排版诗稿 (干支标题 + 目录 + 正文) |
| 数据导出 | 日记按日期导出为合集文本 |
